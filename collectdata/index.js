...
function walkThroughPages(page, records, cb) {
    return nightmare
        ...
        .catch(e => {
            console.error('----------- fail scraping!')
        })
}

function walkThroughInstansies(url, data , cb) {
    ...
    .then(() => {
        walkThroughPages(1, [], scrapped => {
            filterData(`${url}/lelang`, scrapped, [], filtered => {
                data = data.concat(filtered)
                if (instansi[index + 1]) walkThroughInstansies (instansi[index + 1], data, cb)
                else cb(data)
            })
        })
    })
...
function filterData (url, scrapped, filtered, cb)
{
    let obj = scrapped[0]
    let dest = `${url}/${obj.kode}/jadwal`
    nightmare
    .goto(dest)
    .evaluate(() => {
        let start = $('body > div:nth-child(2) > div > table > tbody > tr:nth-child(2) > td:nth-child(3)').html()
        let end = ''
    ...
    .catch(e => {
        console.log('---- fail to get scehdule')
    })
    .then(result => {
...
console.time()
instansi = ['http://lpse.butonkab.go.id/eproc4']
walkThroughInstansies('http://lpse.butonkab.go.id/eproc4', [], data => {
    console.timeEnd()
    nightmare.end().then()
    console.log(data)
})